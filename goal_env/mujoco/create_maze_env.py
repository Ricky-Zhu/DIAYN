...
class GoalWrapper(Wrapper):
    def __init__(self, env, maze_size_scaling, random_start, low, high, fix_goal=True, top_down=False, test=None):
        ...
        self.reset_space = gym.spaces.Box(low=reset_low, high=reset_high)

        # add the reset position which locate the ant in the connector
        connector_low = np.array([1.8, -0.4]) * self.maze_size_scaling
        connector_high = np.array([2.3, 0.3]) * self.maze_size_scaling
        self.connector_reset_space = gym.spaces.Box(low=connector_low, high=connector_high)
    ...
    def get_dataset(self, num_obs=500):
        # get the state data when the ant is in the upper corner
        ...
        sub_steps_count = 0  # to make the data diverse

        while len(obs) < num_obs:
            while self.reset_space.contains(observation[:self.goal_dim]):
        ...
        dataset = {'observations': np.asarray(obs),
    ...
    def reset(self):
        ...
        if self.random_start == 1:
            ...
            xy = self.connector_reset_space.sample()
            while (self.env._is_in_collision(xy)):
                xy = self.connector_reset_space.sample()
            self.env.wrapped_env.set_xy(xy)
            observation = self.env._get_obs()
