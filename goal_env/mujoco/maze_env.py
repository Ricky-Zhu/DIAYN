...
class MazeEnv(gym.Env):
    ...
    def reset(self, goal):
        ...
        if self.visualize_goal:  # remove the prev goal and add a new goal
            ...
            if not self.xml_writer_count:
                _, self.file_path = tempfile.mkstemp(text=True, suffix='.xml')

                self.tree.write(
                    self.file_path)  # here we write a temporal file with the robot specifications. Why not the original one??

            model_cls = self.__class__.MODEL_CLASS
            self.wrapped_env = model_cls(*self.args, file_path=self.file_path,
